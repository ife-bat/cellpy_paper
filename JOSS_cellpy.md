...
Within the large space of different battery testers, testing data formats and variables, 

The Python library `cellpy` was initially created to read battery test data from a few different battery cell testers (with focus on the [Arbin battery cyclers](https://www.arbin.com/)), categorize the data and create summary statistics, and export it in a common format. 
Typically, a battery-testing data set consists of time series data with voltage ($V$), current ($I$) and capacity ($Q$). Additional measurements, such as e.g., temperature or pressure might also be included.
The most common way of structuring battery data is to split the time series data into so-called battery cycles (charge & discharge). Each cycle usually consists of several steps, where for example the current can be constant in one step, and zero in the next step. \autoref{fig:1} illustrates a typical data set containing one full cycle (“cycle”) consisting of four steps (“s1” to “s4”). By looking at specific steps, the battery researcher can for example interpret how the internal resistance in the battery develops over time or say something about the internal kinetics in the battery cell. The battery’s cycle life can be determined by analysing the battery capacity as a function of cycle number.
