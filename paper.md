...
Typically, a battery-testing data set consists of time series data with voltage ($V$), current ($I$) and capacity ($Q$). Most cell cycling instruments provide these data in a format which is straightforward to plot and adjust.

The most common way of structuring battery data is to split the time series data into so-called battery cycles (charge & discharge), where each cycle consists of several steps. \autoref{fig:1} illustrates a typical data set containing one full cycle consisting of four steps (“s1” to “s4”). Analysing individual cyles and steps allows to gain a detailed understanding of the battery cell.

![Illustration of battery cell test data.\label{fig:1}](./Figures/Cell-test-data.jpg)

Unfortunately, different brands of instrument handle the digital information in different ways and store it in different formats. To compare many cells tested under different conditions and with a large variety of additional meta-data (type of cell, type of chemistry, electrode properties, etc.), more advanced data handling methodologies are required. One of the important steps in this process is homogenizing the data, collecting information from multiple sources and ensuring consitancy, accuracy and comparability. This is one of the key functionalities in `cellpy`: `cellpy` can directly parse the data from most commercial battery testing units, converts and saves the data in a common format so that subsequent data handling becomes considerably easier. In addition to translating data to a common format, `cellpy` has a range of utilities for studying the data, including: extraction of key characteristics from the tests, calculation of derivatives, cell comparison, plotting and statistical analysis.
